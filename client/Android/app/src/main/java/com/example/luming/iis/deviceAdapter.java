...
public class deviceAdapter extends ArrayAdapter {
    ...
    public deviceAdapter(MainActivity context, int textViewResourceId, List<Device> objects) {
        super(context, textViewResourceId, objects);
        this.mContext = context;
        dbOperator = DatabaseOperator.getInstance(mContext);
        resourceId = textViewResourceId;
    }
    @Override
    public View getView(int position, View convertView, ViewGroup parent) {
        final Device device = (Device) getItem(position); // 获取当前项的Fruit实例
      ...
      //  ImageView fruitImage = (ImageView) view.findViewById(R.id.fruit_image);//获取该布局内的图片视图
        ...
        final TextView devicePort = (TextView) view.findViewById(R.id.device_port);
        Button deviceConn = (Button) view.findViewById(R.id.device_conn);
        Button deviceDelete = (Button) view.findViewById(R.id.device_delete);
        ...
        deviceConn.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v) {
                mContext.connection(device.getIp(),device.getPort());
            }
        });
